...
import studiesFixture from "../../data/studies.fixture.json";
...
// How long to stall the fixture load so the loading spinners stay visible.
// Set to 0 for an instant load.
const FIXTURE_DELAY_MS = 1200;

// Designs counted as secondary research for the stat tiles. Must match
// SECONDARY_DESIGNS in scripts/generate-fixture.js.
...
const initialStats = [
  {
    color: "red",
    stats: [{ title: "Studies", metric: "--" }],
  },
  {
    color: "orange",
    stats: [{ title: "Participants", metric: "--" }],
  },
  {
    color: "yellow",
    stats: [{ title: "Primary Studies", metric: "--" }],
  },
  {
    color: "green",
    stats: [{ title: "Secondary Studies", metric: "--" }],
  },
  {
    color: "blue",
    stats: [{ title: "Measures", metric: "--" }],
  },
];
...
function DashboardRoute(props) {
  ...
  const onUpdateButtonClicked = (e) => {
    ...
    closeSidebar();
    // On mobile the drawer covers the charts, so applying filters should reveal
    // the result rather than leave the user looking at the filter list.
  ...
  const [initialFilterLoadComplete, setInitialFilterLoadComplete] =
    useState(false);

  // DEMO DATA ACCESS
  // The original Airtable feed is gone — Airtable deprecated the API keys this
  // app shipped with (Feb 2024), so the live fetch returned 401 and every
  // spinner hung forever. Data now comes from a static fixture instead; see
  // scripts/generate-fixture.js to regenerate it.
  ...
  */
    ...
    for (let item of Object.keys(dataholder)) {
      ...
      for (let j of Object.values(records_obj)) {
        ...
        for (let k of j) {
          ...
          if (
            k[groupName].includes(item) &&
            Object.keys(subdict).indexOf(k[barName]) < 0
          ) {
            //console.log("subdict: ", subdict)
            let s = new Set();
            s.add(k);
            subdict[k[barName]] = s;
          } else if (k[groupName].includes(item)) {
            // if the record should be in there AND it isn't already
    ...
    for (var i of uni) {
      ...
      for (let status of list_all) {
        ...
        for (let j of all) {
          ...
          if (
            typeof j[color] !== "undefined" &&
            typeof j[x_axis] !== "undefined" &&
            typeof j[y_axis] !== "undefined"
          ) {
            ...
            if (
              j[x_axis].includes(ids[1]) &&
              j[y_axis].includes(ids[2]) &&
              j[color].includes(status)
            ) {
              l1.push(j);
              //console.log("JJJ")
            }
  ...
  // Mobile only — on desktop the nav is always visible and this is ignored.
  ...
  const [trialRandomizationPieChartData, setTrialRandomizationPieChartData] =
    useState([]);
  ...
  const [cumulativeTrialsLineChartData, setCumulativeTrialsLineChartData] =
    useState([]);
  const [singleMultiSitePieChartData, setSingleMultiSitePieChartData] =
    useState([]);
  ...
  const [
    populationVolunteersPieChartData,
    setPopulationVolunteersPieChartData,
  ] = useState([]);
  const [
    populationEnrollmentPieChartData,
    setPopulationEnrollmentPieChartData,
  ] = useState([]);

  // outcomes charts original varible
  const [outcomesTop10ParentBarChartData, setOutcomesTop10ParentBarChartData] =
    useState({ data: [], group_keys: [] });
  ...
  const [interventionsTop10BarChartData, setInterventionsTop10BarChartData] =
    useState({ data: [], group_keys: [] });
  const [interventionTypesPieChartData, setInterventionTypesPieChartData] =
    useState([]);
  const [interventionsBarData, setInterventionsBarData] = useState({
    data: [],
    group_keys: [],
  });
  const [comparatorsBarData, setComparatorsBarData] = useState({
    data: [],
    group_keys: [],
  });

  const [interventionArmsPieChartData, setInterventionArmsPieChartData] =
    useState([]);
  ...
  const [
    sponsorsTop10ByTrialsBarChartData,
    setSponsorsTop10ByTrialsBarChartData,
  ] = useState({ data: [], group_keys: [] });
  const [sponsorTypePieChartData, setSponsorTypePieChartData] = useState([]);
  const [
    sponsorsTop10ByEnrollmentBarChartData,
    setSponsorsTop10ByEnrollmentBarChartData,
  ] = useState({ data: [], group_keys: [] });
  ...
  const [geographyFacilitiesChartData, setGeographyFacilitiesChartData] =
    useState([]);
  const [countriesTop10BarChartData, setCountriesTop10BarChartData] = useState({
    data: [],
    group_keys: [],
  });
  ...
  const [loadingSponsorsSunburstChart, setLoadingSponsorsSunburstChart] =
    useState(true);
  const [trialsSunburstChart, setTrialsSunburstChart] = useState({});
  const [loadingTrialsSunburstChart, setLoadingTrialsSunburstChart] =
    useState(true);

  const [trialsLandscapeChartData, setTrialsLandscapeChartData] = useState([]);
  const [populationsLandscapeChartData, setPopulationsLandscapeChartData] =
    useState([]);
  const [interventionsLandscapeChartData, setInterventionsLandscapeChartData] =
    useState([]);
  ...
  const [trialsLandscapeMinNodeSize, setTrialsLandscapeMinNodeSize] =
    useState(0);
  const [trialsLandscapeMaxNodeSize, setTrialsLandscapeMaxNodeSize] =
    useState(1);
  const [trialsLandscapeXs, setTrialsLandscapeXs] = useState([]);
  const [populationsLandscapeMinNodeSize, setPopulationsLandscapeMinNodeSize] =
    useState(0);
  const [populationsLandscapeMaxNodeSize, setPopulationsLandscapeMaxNodeSize] =
    useState(1);
  const [
    interventionsLandscapeMinNodeSize,
    setInterventionsLandscapeMinNodeSize,
  ] = useState(0);
  const [
    interventionsLandscapeMaxNodeSize,
    setInterventionsLandscapeMaxNodeSize,
  ] = useState(1);
  const [interventionsYs, setInterventionsYs] = useState([]);
  const [outcomesLandscapeMinNodeSize, setOutcomesLandscapeMinNodeSize] =
    useState(0);
  const [outcomesLandscapeMaxNodeSize, setOutcomesLandscapeMaxNodeSize] =
    useState(1);
  const [sponsorsLandscapeMinNodeSize, setSponsorsLandscapeMinNodeSize] =
    useState(0);
  const [sponsorsLandscapeMaxNodeSize, setSponsorsLandscapeMaxNodeSize] =
    useState(1);
  ...
  const [loadingInterventionsData, setLoadingInterventionsData] =
    useState(true);
  ...
  const [loadingTrialsLandscapeData, setLoadingTrialsLandscapeData] =
    useState(true);
  const [loadingPopulationsLandscapeData, setLoadingPopulationsLandscapeData] =
    useState(true);
  const [
    loadingInterventionsLandscapeData,
    setLoadingInterventionsLandscapeData,
  ] = useState(true);
  const [loadingOutcomesLandscapeData, setLoadingOutcomesLandscapeData] =
    useState(true);

  const [loadingActivityTypesData, setLoadingActivityTypesData] =
    useState(true);
  ...
  const [activityBarChartData, setActivityBarChartData] = useState({
    data: [],
    group_keys: [],
  });
  const [outcomesBarData, setOutcomesBarData] = useState({
    data: [],
    group_keys: [],
  });
  ...
  */
  ...
  // Reads the static demo fixture in place of the old Airtable call, keeping the
  // same { tabledata: [...] } shape so nothing downstream had to change.
  //
  // The artificial delay exists on purpose: it's the only thing that makes the
  // ~25 loading flags and their spinners observable now that the data is
  // local. Set FIXTURE_DELAY_MS to 0 to load instantly.
  async function getTableData() {
    await new Promise((resolve) => setTimeout(resolve, FIXTURE_DELAY_MS));

    // Copy each record — the dashboard mutates rows in place (filterData
    // coerces every field with toString()), and the imported JSON module is a
    // shared singleton that would accumulate those mutations across reloads.
    var table_data = studiesFixture.map(function (record, index) {
      return Object.assign({}, record, { id: index });
    });

    table_data.forEach(function (record) {
      alldata.push(record);
    });
  ...
  function createSingleMetrics(data) {
    ...
    let enrollment = 0;
    let secondary = 0;
    // Distinct outcome measures across the corpus. Outcomes is comma-delimited.
    ...
    for (let record of data) {
      ...
      if (SECONDARY_DESIGNS.includes(record["Design"])) {
        secondary += 1;
      }

      let outcomes = record["Outcomes"];
      if (outcomes !== "" && typeof outcomes !== "undefined") {
        for (let outcome of outcomes.toString().split(",")) {
          let trimmed = outcome.trim();
          if (trimmed !== "") {
            measures.add(trimmed);
          }
        }
      }
    }

    // Primary + secondary always sums to trials.
    let primary = trials - secondary;

    setStats([
      {
        color: "red",
        stats: [{ title: "Studies", metric: trials }],
      },
      {
        color: "orange",
        stats: [{ title: "Participants", metric: enrollment }],
      },
      {
        color: "yellow",
        stats: [{ title: "Primary Studies", metric: primary }],
      },
      {
        color: "green",
        stats: [{ title: "Secondary Studies", metric: secondary }],
      },

      {
        color: "blue",
        stats: [{ title: "Measures", metric: measures.size }],
      },
    ]);
  ...
  function createChoropleth(data) {
    ...
    // Scale the color ramp to the data instead of a hardcoded [0, 300]. With a
    // fixed upper bound far above the real maximum every country lands at the
    // bottom of the ramp and the map reads as one flat color.
    let max_count = 0;
    for (let entry of geog_result) {
  ...
  // Mobile drawer. Closing it also dismisses the filter pane, which sits on top
  // of the drawer — leaving the pane open over a hidden drawer would strand the
  // user with no way back to the category list.
  const closeDrawer = () => {
    setDrawerIsOpen(false);
    closeSidebar();
      ...
      switch (activeCategoryFilter) {
        case "Studies":
          setTrialsFilters((filters) =>
            updateFilters(filters, section, filter),
          );
          setActiveParentFilterSections((filters) =>
            updateFilters(filters, section, filter),
          );
          if (
            (filter || trialsFilters[section][filter] === false) &&
            shouldSelect === true
          ) {
        ...
        case "Populations":
          ...
          if (
            (filter || populationFilters[section][filter] === false) &&
            shouldSelect === true
          ) {
            setActiveParentFilter(filter);
            setActiveChildFilterSections({});
          ...
          break;
        case "Interventions":
          setInterventionsFilters((filters) =>
            updateFilters(filters, section, filter),
          );
          setActiveParentFilterSections((filters) =>
            updateFilters(filters, section, filter),
          ...
          if (
            (filter || interventionsFilters[section][filter] === false) &&
            shouldSelect === true
          ) {
            setActiveParentFilter(filter);
            setActiveChildFilterSections({});
        ...
        case "Outcomes":
          ...
          if (
            (filter || outcomesFilters[section][filter] === false) &&
            shouldSelect === true
          ) {
            setActiveParentFilter(filter);
            setActiveChildFilterSections({});
        ...
        case "Conditions":
          ...
          if (
            (filter || conditionsFilters[section][filter] === false) &&
            shouldSelect === true
          ) {
            setActiveParentFilter(filter);
            setActiveChildFilterSections({});
        ...
        case "Geography":
          ...
          if (
            (filter || geographyFilters[section][filter] === false) &&
            shouldSelect === true
          ) {
            setActiveParentFilter(filter);
            setActiveChildFilterSections({});
  ...
  const showDoubleSideBar =
    (activeParentFilter || hoveredParentFilter) &&
    [].includes(activeCategoryFilter);
  ...
  const [activitiesBar, setActivitiesBar] = useState({
    data: [],
    group_keys: [],
  });
  ...
  const [comparatorsBar, setComparatorsBar] = useState({
    data: [],
    group_keys: [],
  });
  const [loadingComparatorBars, setLoadingComparatorsBar] = useState(true);

  const [conditionsScatter, setConditionsScatter] = useState({
    data: [],
    min: 0,
    max: 1,
  });
  const [loadingConditionsScatter, setLoadingConditionsScatter] =
    useState(true);

  const [interventionsScatter, setInterventionsScatter] = useState({
    data: [],
    min: 0,
    max: 1,
  });
  const [loadingInterventionsScatter, setLoadingInterventionsScatter] =
    useState(true);
  ...
  const [activityMultiBar, setActivityMultiBar] = useState({
    data: [],
    group_keys: [],
  });
  const [loadingActivityMultiBar, setLoadingActivityMultiBar] = useState(true);

  const [yearsScatter, setYearsScatter] = useState({
    data: [],
    min: 0,
    max: 1,
  });
  ...
  useEffect(() => {
    if (allDataLoaded) {
      ...
      setActivitiesBar(
        createBarChart("Interventions", 100, "type", "value", allData),
      );
      ...
      setComparatorsBar(
        createBarChart("Comparator", 100, "type", "value", allData),
      );
      ...
      setActivityMultiBar(
        createMultiBarChart("Conditions", "Activity_Type", allData),
      );
      setLoadingActivityMultiBar(false);

      setYearsScatter(
        createScatterPlot("Year", "Conditions", "Design", "trials", allData),
      );
      ...
      <Container fluid className="dashboard-route">
          ...
          <div className="scrollable-container">
            <Row className="title-block">
              <Col>
                <PrismTextBlock
                  textTitle={
                    "Music in the Treatment and Management of Serious Mental Illness: A Global Scoping Review of the Literature"
                  }
                />
              </Col>
            </Row>
            <Row>
              <Col lg={{ span: 6 }}>
                <PrismTextBlock
                  mainText={
                    <p>
                      <b>Authors</b>: Tasha L. Golden, Stacey Springs, Hannah J.
                      Kimmel, Sonakshi Gupta, Alyssa Tiedemann, Clara C. Sandu,
                      and Susan Magsamen
                    </p>
                  }
                />
                ...
                <PrismTextBlock
                  mainText={
                    <p>
                      <b>Publication</b>:{" "}
                      <a
                        href="https://www.frontiersin.org/articles/10.3389/fpsyg.2021.649840/full"
                        target="_blank"
                      >
                        The Use of Music in the Treatment and Management of
                        Serious Mental Illness: A Global Scoping Review of the
                        Literature.
                      </a>{" "}
                      Frontiers in psychology. 2021 Mar 31;12:880.
                    </p>
                  }
                />
              </Col>
            ...
            <Row>
              <Col lg={{ span: 6 }}>
                <PrismTextBlock
                  ...
                  moreText={
                    "On the left side of the page are sets of data filters. These can be used to focus on specific subsets of the evidence, such as specific populations or conditions."
                  }
                />
              </Col>
              <Col lg={{ span: 6 }}>
                <PrismTextBlock
                  moreText={
                    <iframe
                      width="95%vw"
                      height="315"
                      src="https://www.youtube.com/embed/6NGnFYmskYM?autoplay=1&autohide=0&controls=0&modestbranding=1"
                      frameborder="0"
                    ></iframe>
                  }
                />
            ...
            <Row>
              <Col>
                <PrismTextBlock textTitle={"Data Summary"} />
              </Col>
            </Row>
            ...
            <Row className="d-xl-none" style={{ paddingTop: "10px" }}>
              <Col lg={{ span: 12 }}>
                <div className="single-stats-containers">
                  {stats.map((stat, index) => {
                    if (index >= stats.length / 2) return null;
                ...
                <Row>
                  ...
                  <Col lg={{ span: 6 }}>
                    <PrismTextBlock
                      ...
                      mainText={
                        "Due to the wide variety of study facets involved in this scoping review, results were classified according to whether the music intervention was found to have performed better, worse, equivalent to the comparator, or whether results were undetermined."
                      }
                    />
                  </Col>
                </Row>
                ...
                <Row>
                  ...
                  <Col lg={{ span: 6 }}>
                    <PrismTextBlock
                      ...
                      mainText={
                        "       In research, the word ‘Outcomes’ refers to the variables that an intervention is designed to improve or change."
                      }
                    />
                  </Col>
                </Row>
        ...
        {activeCategoryFilter && (
          <SlidingPane
            ...
            from={"left"}
            hideHeader={true}
            width={isMobile ? "100%" : showDoubleSideBar ? "750px" : "500px"}
            onRequestClose={() => closeSidebar()}
          >
            <div
              className={
                "sliding-pane-container " + colors[activeCategoryFilter]
              }
            >
              {
                // Mobile: the pane covers the drawer, so it needs its own way
                // back to the category list. closeSidebar leaves drawerIsOpen
                // untouched, so dismissing the pane reveals the drawer beneath.
                isMobile && (
                  <div
                    className={
                      "mobile-filter-header " + colors[activeCategoryFilter]
                    }
                  >
                    <button
                      type="button"
                    ...
                    <p className="title">{activeCategoryFilter}</p>
                  </div>
                )
              }
